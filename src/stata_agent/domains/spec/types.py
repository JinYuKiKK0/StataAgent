...
class RequirementParseResult(BaseModel):
    spec: ResearchSpec | None = Field(
        default=None, description="需求解析成功后得到的研究规范"
    )
    raw_response_text: str | None = Field(
        default=None, description="模型原始文本输出，用于审计"
    )
...
class VariableRequirementsResult(BaseModel):
    ...
    data_requirements_draft: DataRequirementsDraft = Field(
        ..., description="数据需求表草案"
    )
