...
- 正在处理：编排层解耦重构（为 `S1-T7` Gateway 审批中断与恢复做前置工程准备），将 S1 节点从主编排器下沉到阶段编排器。
- 阶段：S1（需求解析与最低可行数据契约）
- 分支：main
- 关键文件：
  - `feature_list.json` — `S1-T6` 仍为 `passes: true`；下一优先任务仍为 `S1-T7`（Gateway 审批中断与恢复）。
  ...
  - `src/stata_agent/workflow/ports.py` — 新增 `Phase1OrchestratorPort` / `Phase2OrchestratorPort` / `Phase3OrchestratorPort` 阶段编排端口。
  - `src/stata_agent/workflow/orchestrator.py` — 由节点级实现收敛为阶段委派，仅保留 `phase1_feasibility` 调度与依赖装配。
  - `src/stata_agent/workflow/stages/phase1_feasibility.py` — 承载 S1 五个节点（parse/requirements/map/probe/contract）的状态推进逻辑。
  - `src/stata_agent/workflow/stages/phase2_modeling.py` / `src/stata_agent/workflow/stages/phase3_execution.py` — 建立后续阶段骨架，避免后续节点继续膨胀主编排器。
  - `src/stata_agent/workflow/state.py` / `src/stata_agent/workflow/types.py` — 新增 `data_contract_bundle` 状态字段与 `CONTRACTED` 阶段。
  - `tests/test_workflow_orchestrator.py` / `tests/test_phase1_orchestrator.py` — 保持集成回归并新增阶段单测，验证迁移后行为一致。
