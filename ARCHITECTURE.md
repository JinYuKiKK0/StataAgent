...
`AGENTS.md` 是会话入口和导航地图。本文件只固定顶层系统边界、源码目录结构、分层职责和稳定契约，避免新上下文窗口中的 Agent 擅自迁移包结构。更细的产品流程和工程规则保存在 `docs/` 中，并通过项目私有 skills 按需加载。
...
以下结构是当前唯一允许的顶层源码布局。未先更新本文件前，不得新增平行顶层包、批量迁移 `.py` 模块。
